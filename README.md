...
设置页最上面是一条**固定顶栏**：左边是 APP 名称，右边一个「⋮」，点开只显示版本和包名（只读信息），点别处收起。
...
改完设置保存时，屏幕上会弹一句「设置已生效(推送/拉取)：距离 …/字号 …/透明度 …」——
那串数字就是当前实际生效的值，可以直接和设置页里的控件对照。
...
2. 再切出键盘看一眼屏幕：进程第一次弹出键盘时会弹一句带版本号的提示，比如
   「简易输入法工具栏 0.18.0 已生效：距离 0dp / 字号 10dp / 透明度 65%」。

* 看得见这句 → 模块已经生效了，那句话里的数字就是当前实际生效的值；
* 完全看不见 → 作用域没勾对，或者输入法进程没重启；
* 这句弹得出来、但屏幕上就是没有那条栏 → 换个输入法试试（个别输入法把窗口高度写死了，见下面"已知限制"）。
