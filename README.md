...
* **整条栏默认全透明**，只让输入法的键盘背景透出来——不占视觉重量，充分利用键盘上方那块本来就浪费的空白；
* 按钮是**文字胶囊**：白底、全圆角、深色字，不画图标；
* 点普通按钮**直接执行**；点带菜单的按钮，弹出一张**居中的浅色圆角卡片菜单**，条目之间有细分隔线，点空白处收起。

想改成深色或者别的配色，在设置页改「胶囊按钮底色 / 按钮文字颜色 / 整条栏背景色」即可。
