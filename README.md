...
改完设置点「保存」（顶栏的对勾或页面底部那个都一样）后，设置页会弹一句「已保存并生效」，
键盘开着的话工具栏会立刻按新值重建。屏幕上不再有别的配置提示：**只有失败才提示**。
...
命令跑在后台线程里（最多等 10 秒，不会卡住输入法），结果只用一句 Toast 报出来：
成功就说"命令完成（root/普通身份）"，失败就把退出码和输出的第一行带上。
