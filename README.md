...
设置页走**简约卡片风**：浅灰底 + 白色圆角卡片，一节一张卡（显示设置 / 配色 / 按钮 /
ADB 命令 / 运行日志），卡片之间和页面左右都留出间隙；卡片里是黑字。
浅绿只出现在三处 ——「启用工具栏」的对勾、可拖动的数值条、按钮。
颜色都写在 `res/values/colors.xml` 里。
...
设置页里有张「ADB 命令」卡片：写一条 shell 命令、点「执行命令」，结果直接显示在下面，
同时也写进运行日志。命令跑在后台线程里（最多等 10 秒），所以不会卡住输入法。
