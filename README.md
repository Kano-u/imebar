...
**点击行为：** 普通按钮直接执行；带菜单的按钮会在**该按钮正上方**弹出一张白色圆角卡片（紧凑条目、黑字、条目间有细分隔线），水平方向对齐按钮、靠边时自动夹回窗口内（最右边那个「⋮」也是这么弹的）；点卡片外面收起，再点同一个按钮也收起。
...
* 按钮样式：**纯文字**（无底色、无边框）
* 排列方式：**均分铺满**（按钮等宽横向铺开）
* 最右边：固定一个**「⋮」窄条**（36dp，不参与均分），点开就是**「复制日志 / 清除日志」**——它是内置的，不写进按钮配置、也删不掉
...
日志不在这一栏里：工具栏最右边的「⋮」是内置的，点开就是「复制日志 / 清除日志」。
...
（工具栏最右边的「⋮ → 复制日志」能把它取出来），并用一句 Toast 告诉你成功还是失败。
...
想看这些记录，用工具栏最右边的「⋮ → 复制日志」就行（想干净点就先点「清除日志」再复现一遍）。
