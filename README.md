...
设置页走**简约卡片风**：浅灰底 + 白色圆角卡片（带一点阴影），一节一张卡（显示设置 / 配色 / 按钮 / 运行日志），
卡片之间和页面左右都留出间隙；卡片里是黑字，卡片标题用深一点的蓝。
蓝色（`#2196F3`）只出现在三处 ——「启用工具栏」的对勾、可拖动的数值条、按钮。
颜色都写在 `res/values/colors.xml` 里。**改完点「保存」才生效**，「恢复默认」会填好默认值并立即保存。
