...
|---|---|---|
| 启用工具栏 | 总开关 | 开 |
| 底部距离 | 工具栏距键盘底边的高度 | 12 dp |
| 左右边距 | 工具栏两侧与屏幕边缘的距离 | 15 dp |
| 文字大小 | 文字按钮的字号 | 12 dp |
| 显示透明度 | 整条栏的不透明度 | 80 % |
| 位置 | 键盘底部 / 键盘顶部 | 键盘底部 |
| 按钮样式 | 纯文字 / 胶囊 | 纯文字 |
| 排列方式 | 均分铺满 / 左对齐（条目多时左对齐可横向滚动） | 均分铺满 |
| 文字颜色 / 胶囊底色 / 整条栏背景色 | 配色 | `#202124` / `#F2F3F5` / 全透明 |

这是照着 AI超级工具栏 的「显示设置」那一套定的坐标。它还有一项「图标大小」，而本模块目前只做文字按钮，
所以没有这项设置（以后加图标按钮时会补上）；按钮之间的间距固定 8dp，不做成可调项。
